...
An ndarray is a list that is \textbf{rectangular} (all sublists have the same length) and contains only \textbf{numeric} values (\texttt{int}, \texttt{real}, or \texttt{complex}). Ndarray status is determined automatically -- no type annotation is needed. A list that is rectangular and all-numeric gains ndarray operations; all other lists remain standard lists.

\paragraph{Detection Rules}
A list is an ndarray if and only if it satisfies all of the following:
\begin{enumerate}
\item All elements are \texttt{int}, \texttt{real}, or \texttt{complex} (no strings, bools, or other types).
...
\paragraph{Examples}
\begin{itemize}
\item \texttt{[1, 2, 3]} — ndarray (1D, shape (3))
\item \texttt{[1+2i, 3+4i]} — ndarray (1D, shape (2), complex)
\item \texttt{[[1,2], [3,4], [5,6]]} — ndarray (2D, shape (3,2))
\item \texttt{[[1+2i, 3], [4, 5+6i]]} — ndarray (2D, shape (2,2), complex)
\item \texttt{[[1,2], [3,4,5]]} — NOT ndarray (different lengths)
\item \texttt{[1, "hello", True]} — NOT ndarray (non-numeric)
\item \texttt{[]} — ndarray (1D, shape (0))
\end{itemize}
...
\item Ndarrays cannot contain function values (ndarray elements must be numeric)
\end{itemize}
