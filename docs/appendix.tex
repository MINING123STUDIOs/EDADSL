...
$prod t= $a * $c          #c# 20 + 30j (int * complex -> complex)
...
$sum     t= $z1 + $z2         # 4 + 2j
$product t= $z1 * $z2         # 11 - 2j
$mag     t= f.math.abs($z1)         # 5.0
$conj_z  t= f.cplx.conj($z2)        # 1 + 2j
...
\begin{tikzpicture}[
  ...
  \node (complex) [above of=real] {complex\\[-2pt]\texttt{3 + 4j}};
...
\begin{tikzpicture}[
  ...
  \node[level, fill=orange!20] (p0) {1. Exponentiation};
  \node[level, fill=yellow!20] (p1) [below of=p0] {2. Boolean NOT, Set Cardinality};
  \node[level, fill=green!20] (p2) [below of=p1] {3. Mul, Div, Mod, Matrix};
  \node[level, fill=cyan!20]  (p3) [below of=p2] {4. Shift, Rotate};
  \node[level, fill=blue!20]  (p4) [below of=p3] {5. Add, Sub};
  \node[level, fill=blue!20]  (p5) [below of=p4] {6. Comparison};
  \node[level, fill=purple!20] (p6) [below of=p5] {7. Logical AND, NAND};
  \node[level, fill=pink!20]  (p7) [below of=p6] {8. Logical OR, NOR};
  \node[level, fill=red!10]  (p8) [below of=p7] {9. XOR, XNOR, IMPLY, NIMPLY};
  ...
  \node[draw=none, font=\small, right of=p1, xshift=4cm] {\texttt{! ||a}};
  \node[draw=none, font=\small, right of=p2, xshift=4cm] {\texttt{* / // \% .* .\^{}}};
  ...
  \node[draw=none, font=\small, right of=p8, xshift=4cm] {\texttt{\^{}}\texttt{\^{}} \texttt{!}\texttt{\^{}} \texttt{->} \texttt{!->}};

  % Label
  \node[draw=none, font=\small\bfseries, above of=p0, yshift=0.4cm] {Highest};
  \node[draw=none, font=\small\bfseries, below of=p8, yshift=-0.4cm] {Lowest};
\end{tikzpicture}
\end{center}
