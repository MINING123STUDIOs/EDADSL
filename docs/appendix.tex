...
$hyp t= f.sqrt($a^2 + $b^2) #c# ~10.49 (real)
$ang t= f.angle($c)          #c# ~0.9828 rad (real)

f.print(f.str("a + b = ", $sum))
f.print(f.str("a * c = ", $prod))
f.print(f.str("hypotenuse = ", $hyp))
f.print(f.str("angle = ", $ang))

halt
\end{verbatim}

\subsection{Explanation}
Variables are declared with explicit types. Arithmetic operations implicitly promote to the wider type: int + real yields real, int + complex yields complex. The \texttt{f.angle} function returns the argument (phase) of a complex number in radians.
...
f.print(f.str($value, " ohms: ", $series, ", ", $tolerance))

halt
\end{verbatim}

\subsection{Explanation}
The \texttt{if}/\texttt{otherwise}/\texttt{else} chain classifies the resistor into ranges. Use \texttt{otherwise} for else-if conditions. There is no \texttt{match}/\texttt{case} statement; equality checks use nested \texttt{if} with \texttt{==}. The expected output is \texttt{4700.0 ohms: E24 common, standard}.

\section{Lambdas and Higher-Order Functions}
\subsection{Intent}
Use lambda expressions with \texttt{f.map}, \texttt{f.filter}, and \texttt{f.reduce} to process a list of resistor values.
...
$powers [list]= f.map($resistors,
    L::($r [real]) -> { (5.0^2) / $r })
f.print(f.str("Powers: ", $powers))

# Filter to only resistors under 1k
$small [list]= f.filter($resistors,
    L::($r [real]) -> { $r < 1000 })
f.print(f.str("Under 1k: ", $small))

# Sum all power dissipation
$total_power [real]= f.reduce($powers,
    L::($acc [real], $p [real]) -> { $acc + $p },
    0.0)
f.print(f.str("Total power: ", $total_power, " W"))

# Sort resistors ascending
$sorted [list]= f.sort($resistors,
    L::($a [real], $b [real]) -> { $a - $b })
f.print(f.str("Sorted: ", $sorted))

halt
\end{verbatim}

\subsection{Explanation}
Lambdas are declared with \texttt{L::} prefix, typed parameters, and \texttt->\{\} body. \texttt{f.map} applies a function to each element, \texttt{f.filter} selects elements satisfying a predicate, and \texttt{f.reduce} folds the list with an accumulator. The sort comparator returns negative if the first argument should come first.
...
$mag     t= f.abs($z1)         # 5.0
$conj_z  t= f.conj($z2)        # 1 + 2i

f.print(f.str("|z1| = ", $mag))
f.print(f.str("z1 * z2 = ", $product))
f.print(f.str("conj(z2) = ", $conj_z))
...
$t [list]= [0:1:$N - 1]
$signal [list]= f.map($t,
    L::($n [int]) -> { f.sin(2 * c.pi * 50
        * $n / $fs)
        + 0.5 * f.sin(2 * c.pi * 120
        * $n / $fs) })

# Compute FFT and extract magnitudes
$spectrum [list]= f.fft($signal)
$mags [list]= f.map($spectrum,
    L::($z [complex]) -> { f.abs($z) })

# Find the dominant frequency
$max_mag [real]= f.max($mags)
$peak_index [int]= f.argmax($mags)
$freq [real]= ($peak_index * $fs) / $N

f.print(f.str("Dominant frequency: ", $freq, " Hz"))
f.print(f.str("Magnitude: ", $max_mag))

halt
\end{verbatim}

\subsection{Explanation}
Complex numbers support all arithmetic operators. The \texttt{f.fft} function returns a list of complex values; \texttt{f.map} with \texttt{f.abs} extracts magnitudes. The dominant frequency is recovered by finding the index of the peak magnitude and scaling by $f_s / N$. The expected dominant frequency is 50 Hz.
...
f.print(f.str("Solution x = ", $x))

# Verify: A .* x should equal b
$check [list]= $A .* $x
f.print(f.str("Verification A .* x = ", $check))

# Matrix properties
f.print(f.str("det(A) = ", f.det($A)))
f.print(f.str("rank(A) = ", f.rank($A)))

halt
\end{verbatim}

\subsection{Explanation}
Ndarrays are detected automatically from rectangular all-numeric lists; no type annotation is needed. The \texttt{.*} operator performs matrix multiplication, and \texttt{f.inv} computes the matrix inverse. Functions \texttt{f.det} and \texttt{f.rank} return the determinant and rank respectively. The expected solution is $x = [6.2, -1.4]$.
...
$threshold [real]= 1000
$high [list]= f.filter($resistors,
    L::($v [real]) -> { $v > $threshold })
f.print(f.str("Above 1k: ", $high))

# Filter: keep resistors above 4.7k
$threshold = 4700
$vhigh [list]= f.filter($resistors,
    L::($v [real]) -> { $v > $threshold })
f.print(f.str("Above 4.7k: ", $vhigh))

# Element-wise add scalar to each list element
$offset [int]= 10
$shifted [list]= f.map([10, 20, 30],
    L::($i [int]) -> { $i + $offset })
f.print(f.str("Shifted: ", $shifted))

halt
\end{verbatim}

\subsection{Explanation}
Lambdas capture variables from the enclosing scope. Here \texttt{\$threshold} is captured by reference: reassigning it before calling \texttt{f.filter} changes which values pass the filter. The \texttt{f.map} lambda captures \texttt{\$offset} to add a constant to each element.
...
$t [list]= [0:1:$N - 1]
$clean [list]= f.map($t,
    L::($n [int]) -> { f.sin(2 * c.pi
        * 100 * $n / $fs) })

# Step 2: Add noise (simple LCG pseudo-random)
$noise [list]= f.map($t,
    L::($n [int]) -> { 0.3 * f.sin(2 * c.pi
        * 7 * $n) })
$noisy [list]= f.map($t,
    L::($i [int]) -> { $clean[$i] + $noise[$i] })

# Step 3: Compute FFT of noisy signal
$spectrum [list]= f.fft($noisy)
$mags [list]= f.map($spectrum,
    L::($z [complex]) -> { f.abs($z) })

# Step 4: Find peak frequency
$peak_idx [int]= f.argmax($mags)
$peak_freq [real]= ($peak_idx * $fs) / $N
f.print(f.str("Detected frequency: ", $peak_freq, " Hz"))

# Step 5: Compute signal statistics
$sig_rms [real]= f.rms($noisy)
$sig_peak [real]= f.peak($noisy)
f.print(f.str("RMS: ", $sig_rms, ", Peak: ", $sig_peak))
...
f.print(f.str("Spectrum bins (first 5): ",
    $spec_pairs[0:5]))
...
\subsection{Explanation}
This example chains together multiple language features: \texttt{f.map} for list comprehensions, complex FFT, higher-order functions for magnitude extraction, signal statistics (\texttt{f.rms}, \texttt{f.peak}), and list slicing. The pipeline demonstrates a realistic signal analysis workflow from raw data through frequency-domain representation.
...
\begin{tikzpicture}[
  ...
  \node (ndarray) [below of=list] {ndarray\\[-2pt]\texttt{1;2;3;4}};
\end{tikzpicture}
\end{center}
...
\item An \texttt{ndarray} is a \texttt{list} that is rectangular and all-numeric; detected automatically
\item \texttt{list} element types are polymorphic; \texttt{ndarray} elements are restricted to \texttt{int} or \texttt{real}
...
\begin{tikzpicture}[
  ...
  \node[level, fill=red!20]    (p1) {1. Exponentiation};
  \node[level, fill=orange!20] (p2) [below of=p1] {2. Boolean NOT, Set Cardinality};
  \node[level, fill=yellow!20](p3) [below of=p2] {3. Mul, Div, Mod, Matrix};
  \node[level, fill=green!20]  (p4) [below of=p3] {4. Shift, Rotate};
  \node[level, fill=cyan!20]   (p5) [below of=p4] {5. Add, Sub};
  \node[level, fill=blue!20]   (p6) [below of=p5] {6. Comparison};
  \node[level, fill=purple!20] (p7) [below of=p6] {7. Logical AND, NAND};
  \node[level, fill=pink!20]   (p8) [below of=p7] {8. Logical OR, NOR};
  \node[level, fill=red!10]    (p9) [below of=p8] {9. XOR, XNOR, IMPLY, NIMPLY};
  ...
  % Label
  \node[draw=none, font=\small\bfseries, above of=p1, yshift=0.4cm] {Highest};
...
\item \textbf{Broadcasting} -- Scalars are broadcast to match the ndarray shape
