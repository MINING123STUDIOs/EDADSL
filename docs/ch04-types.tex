...
SI prefixes >= k and scientific notation produce integers when the result is numerically equal to an integer:
\begin{verbatim}
6.2k    #c# = 6200 (int)
1M      #c# = 1000000 (int)
...
\item \texttt{real} values are truncated (e.g., \texttt{f.int(3.7)} becomes \texttt{3})
\end{itemize}
...
\begin{enumerate}
\item All elements are \texttt{int} or \texttt{real} (no strings, bools, or other types).
...
is_ndarray(list):
    ...
    if all elements are int or real: return True  (1D ndarray)
...
\item Postfix increment/decrement (\texttt{\$var++}, \texttt{\$var--}), transpose (\texttt{.T})
