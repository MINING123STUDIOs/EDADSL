...
Note: \texttt{3 + 4i} is a single complex literal in rectangular form. The \texttt{+} is part of the literal, not the addition operator.
...
\item Function return values (e.g., \texttt{f.linalg.inv}, \texttt{f.arr.zeros})
\end{itemize}

\paragraph{Shape}
The shape of a Tensor is described by its dimensions:
\begin{itemize}
...
\item \texttt{|[1+2i, 3+4i]} — valid tensor (1D, shape (2), complex)
\item \texttt{|[[1,2], [3,4], [5,6]]} — valid tensor (2D, shape (3,2))
\item \texttt{|[[1+2i, 3], [4, 5+6i]]} — valid tensor (2D, shape (2,2), complex)
...
Tensors support special operators prefixed with \texttt{.}:
\begin{itemize}
\item \texttt{.*} — matrix multiply
\item \texttt{.T} — transpose
\item \texttt{.H} — conjugate transpose
\item \texttt{.^T} — tensor power (transpose)
\item \texttt{.^H} — tensor power (conjugate transpose)
\item \texttt{.det} — determinant
\item \texttt{.inv} — inverse
\item \texttt{.norm} — norm
\item \texttt{.rank} — rank
\item \texttt{.trace} — trace
\end{itemize}
See Section~\ref{sec:operators} for detailed operator descriptions.

\paragraph{Tensor Functions}
Tensors have dedicated functions in the \texttt{f.arr}, \texttt{f.stat}, and \texttt{f.linalg} namespaces:
\begin{itemize}
\item Creation: \texttt{f.arr.zeros}, \texttt{f.arr.ones}, \texttt{f.arr.identity}, \texttt{f.arr.full}, \texttt{f.arr.linspace}, \texttt{f.arr.arange}
\item Inspection: \texttt{f.stat.shape}, \texttt{f.stat.rank}, \texttt{f.stat.size}, \texttt{f.stat.flatten}, \texttt{f.stat.clone}, \texttt{f.stat.typeof}
\item Statistics: \texttt{f.stat.mean}, \texttt{f.stat.std}, \texttt{f.stat.var}, \texttt{f.stat.sum}, \texttt{f.stat.product}, \texttt{f.stat.min}, \texttt{f.stat.max}, \texttt{f.stat.argmin}, \texttt{f.stat.argmax}
\item Linear algebra: \texttt{f.linalg.inv}, \texttt{f.linalg.det}, \texttt{f.linalg.rank}, \texttt{f.linalg.trace}, \texttt{f.linalg.solve}
\end{itemize}
...
\item Postfix increment/decrement (\texttt{\$var++}, \texttt{\$var--}), transpose (\texttt{.T}), Hermitian transpose (\texttt{.H}), erase (\texttt{~.E})
