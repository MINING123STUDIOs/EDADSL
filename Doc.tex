...
Important: Angles are given in Radians by Default!\\
The Regular Trigonometric Functions (Sine, Cosine and Tangent) ( denoted as \texttt{sin, cos, tan} ) return the Sine Cosine and Tangent of the Argument. \\
Morphology of the Trigonometric Functions:
\begin{verbatim}
EBNF:
( "sin" | "cos" | "tan" ), "(", argument, [ ",", "unit", "=", unit ] ")"

unit = ( ( "'", ("rad" | "deg" ), "'" ) | ( '"', ("rad" | "deg" ), '"' ) )
\end{verbatim}
If the Type of the Argument does not match the expected Type (a Scalar Value or a List of Scalar Values) a Value Error is raised.\\
Example:
\begin{verbatim}
...
It is only defined for $(\mathbb{R} \backslash \{ n\pi\cdot 1\text{rad} : n \in \mathbb{Z} \} )^n$

\subsubsection{Regular Inverse Trigonometric Functions}
\subsubsection{Hyperbolic Trigonometric Functions}
\subsubsection{Hyperbolic Inverse Trigonometric Functions}
