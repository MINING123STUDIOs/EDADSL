...
$my_set [set]= f.set( $my_list )  # { 1, 2, 3 }
\end{verbatim}
...
Examples:
\begin{verbatim}
$r1 [list]= [0:2:10]      # [0, 2, 4, 6, 8, 10]
$r2 [list]= [0:5]         # [0, 1, 2, 3, 4, 5]
$r3 [list]= [0|1|5]       # [0, 0.25, 0.5, 0.75, 1]
\end{verbatim}
...
\begin{verbatim}
$a t= 3              # Equivalent to $a [int]= 3
$name t= "EDADSL"    # Equivalent to $name [string]= "EDADSL"
$flag t= True        # Equivalent to $flag [bool]= True
$colors t= [1, 2, 3] # Equivalent to $colors [list]= [1, 2, 3]
\end{verbatim}
...
$a [int]= 3
$b [int]= 5
$result [int]= $a + $b        # Scalar assignment

$name [string]= "EDADSL"         # String assignment
$flag [bool]= True               # Boolean assignment

$colors [list]= ["red", "green", "blue"]  # List
$hwset [set]= { Q1, Q2, R5 }              # Set
\end{verbatim}
...
\item Multiplication (\texttt{*}), Division (\texttt{/}), Floor Division (\texttt{//}), Modulo (\texttt{\%})
...
Mixed-type operations promote to \texttt{real}. Division by zero raises a runtime error. Floor division (\texttt{//}) truncates toward negative infinity. Modulo uses truncated division (\texttt{sign(result) = sign(dividend)}). Exponentiation is right-associative: \texttt{-2\^{}3 = -8}.
...
# Numeric equality (cross-type)
5 == 5.0           # = False (strict: int vs real)
5 =n= 5.0          # = True  (numeric: same value)
5 =n= 5.1          # = False (different values)
3.14 =n= 3.14      # = True  (real vs real works too)

# Three-way numeric compare
5 >< 3             # = -1  (5 > 3)
5 >< 5             # =  0  (5 =n= 5)
5 >< 7             # =  1  (5 < 7)
5 >< 5.0           # =  0  (numeric equality)
3.14 >< 3.14       # =  0  (real vs real)

# Approximate equality
syst.flag.approx_eps [real]= 0.01
3.0 ~ 3.005        # = True (within 0.01)
3.0 ~ 3.1          # = False (not within 0.01)
3.0 ~+(0.2)~ 3.1    # = True (within 0.2)
3.0 ~+(0.05)~ 3.1   # = False (not within 0.05)

# Multiplicative approximate equality (relative tolerance)
100 ~*(0.01)~ 100.5  # = True (within 1%)
100 ~*(0.001)~ 100.5 # = False (not within 0.1%)

# Multiplicative approximate equality with system default
...
Examples:
\begin{verbatim}
# Dot product (n-dimensional)
[1, 2, 3] . [4, 5, 6]     # = 32 (1*4 + 2*5 + 3*6)
[1, 2] . [3, 4]           # = 11 (2D)
[1, 2, 3, 4] . [5, 6, 7, 8] # = 70 (4D)
...
\item \texttt{\$var++}, \texttt{\$var--} -- Postfix increment / decrement
\item \texttt{||a} -- Cardinality / length
\item \texttt{*}, \texttt{/}, \texttt{//}, \texttt{\%} -- Multiplication, division, floor division, modulo
\item \texttt{+}, \texttt{-} -- Addition, subtraction
\item \texttt{<}, \texttt{<=}, \texttt{==}, \texttt{!=}, \texttt{>=}, \texttt{>}, \texttt{\textasciitilde}, \texttt{\textasciitilde+()} -- Comparison
...
Examples:
\begin{verbatim}
softint(42)       # Returns 42 (already int)
softint(5.0)      # Returns 5 (real equal to int)
softint(3.7)      # Returns 3.7 (not equal to int, unchanged)
softint("hello")  # Returns "hello" (not numeric, unchanged)
\end{verbatim}
...
Example:
$result [int]= f.u.add( 3, 5 )   # $result = 8
\end{verbatim}
...
function f.u.doublen()::{
    $x [int]= $x * 2  # Local $x shadows global
    f.print($x)            # Prints: 20
}

f.u.doublen()
f.print($x)          # Prints: 10 (global unchanged)

function f.u.addLocal( $a, $b )::{
    $sum [int]= $a + $b   # $sum is local
    $retval [int]= $sum
}
...
$result [int]= f.u.factorial( 5 )  # = 120
\end{verbatim}
